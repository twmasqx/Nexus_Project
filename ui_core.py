...
def _device_type_icon(device: Device) -> str:
    """تحديد أيقونة نوع الجهاز من OS أو المنافذ"""
    os_guess = getattr(device, 'os_guess', '').lower()
    if 'windows' in os_guess or 'router' in os_guess or 'nas' in os_guess:
        return DEVICE_TYPE_ICONS['router'] if 'router' in os_guess or 'nas' in os_guess else DEVICE_TYPE_ICONS['computer']
    if 'android' in os_guess or 'ios' in os_guess:
        return DEVICE_TYPE_ICONS['phone']
    return DEVICE_TYPE_ICONS['default']


# أسماء العلامات التجارية للعرض النصي فقط (بدون إيموجي)
VENDOR_ICONS = {
    'APPLE': 'APPLE',
    'SAMSUNG': 'SAMSUNG',
    'HUAWEI': 'HUAWEI',
...
# ألوان Cybersecurity - أخضر فوسفوري وتدرجات
COLORS = {
    'bg_dark': (0.02, 0.03, 0.05, 1),
    'card_bg': (0.06, 0.08, 0.1, 0.85),
    'glass': (0.08, 0.12, 0.15, 0.6),
    'accent': (0.2, 1, 0.5, 1),
    'accent_dim': (0.15, 0.8, 0.4, 0.6),
    'phosphor': (0.4, 1, 0.6, 1),
    'danger': (0.95, 0.2, 0.2, 1),
    'text': (0.95, 0.98, 1, 1),
    'text_dim': (0.65, 0.7, 0.75, 1),
}

# أيقونات نوع الجهاز (هاتف، كمبيوتر، روتر)
DEVICE_TYPE_ICONS = {
    'phone': 'cellphone',
    'computer': 'laptop',
    'router': 'router-wireless',
    'tablet': 'tablet',
    'default': 'cellphone-link',
}
...
class DeviceListItem(MDBoxLayout):
    """بطاقة Glassmorphism - نوع الجهاز + سرعة حية + Slider"""
    def __init__(self, device: Device, engine: NetworkEngine = None, on_click=None, on_speed_change=None, **kwargs):
        ...
        speed_txt = f"[color=39ff66]↓{rx:.0f} ↑{tx:.0f} KB/s[/color]"
        self.speed_label = MDLabel(
            text=f"{device.vendor} • {model}  {speed_txt}",
            markup=True,
            halign='left',
        ...
        # زر كتم/إعادة اتصال يمكن ربطه لاحقاً بـ iptables بدون استخدام إيموجي
        if MDSwitch:
            self.mute_switch = MDSwitch(active=not self._muted, size_hint_x=None, width=dp(44))
            self.mute_switch.bind(active=self._on_mute_toggle)
            row.add_widget(self.mute_switch)
        self.add_widget(row)

        row2 = MDBoxLayout(orientation='horizontal', size_hint_y=None, height=dp(32))
        limit = getattr(device, 'speed_limit_down_kbps', 0)
        self.slider = MDSlider(min=0, max=1000, value=limit, size_hint_x=0.85)
        self.slider.bind(value=self._on_slider_change)
        row2.add_widget(self.slider)
        self.add_widget(row2)

        self.limit_label = MDLabel(
        ...
        self.add_widget(self.limit_label)
    ...
    def _on_mute_toggle(self, instance, value):
        """Switch Mute/Connect - قطع أو إعادة اتصال الجهاز"""
        self._muted = not value
        if hasattr(self.device, 'muted'):
            self.device.muted = self._muted
        if self.engine and is_root_available():
            if self._muted:
                self.engine.kick_device(self.device.ip, self.device.mac)
            else:
                self.engine.set_device_bandwidth_limit(self.device.ip, self.device.mac, 0, 0)
        if self.on_mute_cb:
            self.on_mute_cb(self.device.mac, self._muted)
    ...
    def update_speed_display(self, rx_bps, tx_bps):
        ...
        self.speed_label.text = f"{self.device.vendor} • {model}  [color=39ff66]↓{rx:.0f} ↑{tx:.0f} KB/s[/color]"
...
class NexusVisionApp(MDApp):
    ...
    def build(self):
        ...
        root.bind(pos=lambda *a: setattr(root._bg_rect, 'pos', root.pos) if hasattr(root, '_bg_rect') else None)
        root.bind(size=lambda *a: setattr(root._bg_rect, 'size', root.size) if hasattr(root, '_bg_rect') else None)
        ...
        self.total_down_lbl = MDLabel(text="↓ 0 KB/s", font_style='H6', theme_text_color='Custom', text_color=COLORS['phosphor'])
        ...
        # حالة Root + عداد الأجهزة (بدون إيموجي)
        root_status = "Root: ON" if is_root_available() else "Root: OFF"
    ...
    def _on_intruder_detected(self, device: Device):
        """تنبيه فوري عند دخول جهاز غير معروف (بدون إيموجي)"""
        def _notify():
            try:
        ...
        self._show_snackbar(f'Intruder detected: {device.ip} ({device.vendor})')
    ...
    def _open_security_audit(self, *a):
        ...
        status = 'Secure' if result.get('secure', True) else 'Not Secure'
    ...
    def _on_device_clicked(self, mac: str):
        ...
        txt = f"{d.vendor if d else 'Device'} - {model}\n\n"
        ...
        def build_tree():
            ...
            if self.engine:
                for d in getattr(self.engine, 'devices', []):
                    ...
                    item = TwoLineAvatarListItem(text=f"{ip} | {vendor}", secondary_text=mac)
                    item.add_widget(IconLeftWidget(icon=_device_type_icon(d)))
